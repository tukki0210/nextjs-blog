...
クリックしたら◯か×に変わるという機能は比較的簡単だが、勝敗を判定する部分が少し難しい。<br/>
素直にif文を使い条件分岐を書き込みまくるのも一つの手だが、配列専用のメソッドである**every()**を使うと良いです。<br/>
...
**アロー**というのは **=>** のマークが矢印に見えるからアロー関数という名前がついています。
